...
export default class AlfaComponentRadio extends LightningElement {
  @api storeName;
  @api columnName;
  @api label;
  @api vertical = false;
  ...
  getInnerClass = state => {
    const visible = getStoreData(state.control.visibility, this.columnName, true);
    if (visible === false) {
      return 'slds-hide';
    }
  ...
  getDisabled = state => {
    ...
    return state.control.disabled.allColumns === true || getStoreRadio(state.control.disabled, key);
  };
