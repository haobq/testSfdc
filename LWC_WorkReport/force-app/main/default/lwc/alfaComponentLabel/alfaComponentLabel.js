...
import { CONSTANTS } from 'c/epccConstants';
import {
  regist,
  isEmpty,
  getStoreData,
...
export default class AlfaComponentLabel extends LightningElement {
  @api storeName;
  @api label;
  @api value;
  @api fixHeight = 0;
  ...
  get innerstyle() {
    return (this.fixHeight > 0 && isEmpty(this.innerValue)) ? 'height:' + this.fixHeight + 'px' : '';
  }
  ...
  getColumnValue = state => {
    return isEmpty(this.value) ? getStoreData(state.data, this.columnName) : this.value;
  };
